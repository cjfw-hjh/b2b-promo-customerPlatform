...
- **완료 조건**:
  - [x] 저장 성공 시 BE-6에 실제로 영업일지가 생성된다.
  - [x] 활동 내역 입력란 위/아래에 "가격·계약조건이 아닌 관계 활동을 적어달라"는 안내 문구가 노출된다(PRINCIPLE-ACTIVITY-001).
  - [x] 기존 영업일지 수정 진입 시 기존 값이 폼에 채워지고, 저장 시 PATCH가 호출된다.
...
- **완료 조건**:
  - [x] 검색 조건 입력 후 BE-10 API 결과가 목록에 반영된다.
  - [x] 목록 항목 클릭 시 FE-5(상세 화면)로 이동한다.
  - [x] 상태 배지("작성 완료"/"코멘트 진행중")가 서버 응답값 그대로 표시된다.
...
- **완료 조건**:
  - [x] 코멘트 스레드가 시간순으로 표시되고, 답변 등록 시 즉시 스레드에 반영된다.
  - [x] 코멘트가 있는 영업일지에서 삭제 버튼 클릭이 비활성화되어 있음을 화면에서 확인한다.
  - [x] 코멘트가 없는 영업일지에서는 답변 입력창이 비활성화되어 있다.
...
- **완료 조건**:
  - [x] 로그인한 팀장에게 매핑된 영업사원의 일지만 목록에 나타난다.
  - [x] 목록 클릭 시 FE-7(팀장용 상세)로 이동한다.
...
- **완료 조건**:
  - [x] 영업일지 자체를 수정/삭제하는 버튼이 이 화면에는 존재하지 않는다(팀장 권한 없음, FE-5와 대비).
  - [x] 코멘트 등록 시 즉시 스레드에 반영되고, 여러 번 등록해도 전부 성공한다.
...
- **완료 조건**:
  - [x] 좁은 화면(모바일 폭)에서 모든 목록형 화면이 카드형으로 전환됨을 브라우저 반응형 모드로 확인한다.
  - [x] 검색 조건/폼 입력 필드가 좁은 화면에서 세로로 쌓인다.
...
- **완료 조건**:
  - [x] 음성 인식 결과가 활동 내역 입력란에 반영되고, 저장 전 자유롭게 수정 가능하다.
  - [x] 직접 입력한 텍스트와 음성 입력 결과가 한 입력란에서 자연스럽게 공존한다.
  - [x] `SpeechRecognition` 미지원 브라우저에서도 폼 작성/저장 자체는 정상 동작한다.
