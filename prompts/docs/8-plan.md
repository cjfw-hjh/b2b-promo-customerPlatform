...
- **완료 조건**:
  - [ ] `npm run dev`(또는 동등 스크립트)로 서버가 기동되고 `GET /health` 같은 헬스체크 응답이 200을 반환한다.
  - [ ] `db/pool.js`를 통해 DB-1의 DB에 커넥션이 정상적으로 맺어진다(콘솔 로그로 확인).
  - [ ] ESLint + Prettier 설정이 레포 루트에 있고 `npm run lint`가 통과한다(4-project-principle.md 4번).
...
- **완료 조건**:
  - [ ] 로그인 후 발급된 세션 쿠키가 `httpOnly`로 설정되어 있다(브라우저 개발자도구로 확인).
  - [ ] 세션 데이터가 `connect-pg-simple`의 세션 테이블에 실제로 저장된다(DB에서 직접 조회 확인).
  - [ ] 로그인하지 않은 상태로 보호된 라우트 호출 시 401이 반환된다.
  - [ ] `role`이 다른 사용자가 접근 제한된 라우트(예: 팀장 전용) 호출 시 403이 반환된다.
...
- **완료 조건**:
  - [ ] 정상 입력값으로 영업사원/팀장 각각 회원가입이 성공한다.
  - [ ] 6자리가 아닌 사번, 형식이 잘못된 이메일, 7자리 미만 비밀번호로 가입 시도 시 각각 400이 반환된다.
  - [ ] 동일 사번/이메일로 중복 가입 시도 시 실패한다.
  - [ ] 영업사원 가입 시 `manager_email` 없이 요청하면 실패하고, 팀장 가입 시 `manager_email`을 보내도 무시되거나 저장되지 않는다.
  - [ ] DB에서 `password_hash` 컬럼 값이 평문이 아님을 육안으로 확인한다.
  - [ ] 로그인 성공 시 세션이 생성되고, 로그아웃 시 세션이 즉시 제거된다.
...
- **완료 조건**:
  - [ ] 단위테스트: 영업사원 1명은 `manager_id` 컬럼 값이 최대 1개임을 검증하는 테스트가 통과한다(RULE-ORG-007).
  - [ ] 팀장 계정으로 자신에게 매핑되지 않은 영업사원의 데이터를 조회하는 헬퍼 호출 시 빈 결과가 반환됨을 테스트로 확인한다.
...
- **완료 조건**:
  - [ ] `GET /api/customers` 호출 시 DB-3에서 시드한 거래처 목록이 반환된다.
  - [ ] `POST /api/customers` 같은 등록용 라우트가 존재하지 않음을 코드 리뷰로 확인한다.
...
- **완료 조건**:
  - [ ] 작성 API 호출 시 `created_at`이 서버 시간으로 자동 설정되고 요청 바디로 덮어쓸 수 없다.
  - [ ] 작성자가 아닌 사용자가 수정/삭제를 시도하면 403이 반환된다.
  - [ ] 단위테스트: 코멘트가 1건 이상 있는 영업일지에 DELETE 요청 시 403이 반환된다(RULE-LOG-005).
  - [ ] 단위테스트: 코멘트가 있어도 PATCH(수정) 요청은 성공한다(RULE-LOG-005 — 삭제만 막힘).
  - [ ] 수정 후에도 `created_at` 값이 최초 저장 시점과 동일함을 확인한다(RULE-LOG-004).
  - [ ] 응답 객체에 상태값("작성 완료"/"코멘트 진행중")이 `comments` 개수 기준으로 정확히 계산되어 내려온다.
...
- **완료 조건**:
  - [ ] 팀장 계정으로 호출 시 자신에게 매핑된 영업사원의 일지만 반환된다.
  - [ ] 다른 팀장에게 매핑된 영업사원의 일지는 결과에 포함되지 않음을 테스트로 확인한다.
  - [ ] 영업사원 계정으로 이 엔드포인트 호출 시 403이 반환된다.
...
- **완료 조건**:
  - [ ] 팀장이 아닌 사용자가 코멘트 작성을 시도하면 403이 반환된다.
  - [ ] 코멘트가 하나도 없는 영업일지에 영업사원이 답변을 시도하면 실패한다(RULE-REPLY-001).
  - [ ] 팀장 코멘트 등록 후 같은 영업사원이 여러 번 답변해도 전부 성공한다(횟수 제한 없음).
  - [ ] 팀장도 같은 영업일지에 여러 번 코멘트를 남길 수 있다(RULE-FEEDBACK-001).
  - [ ] `GET /api/managed/comments` 호출 시 해당 팀장이 과거에 남긴 코멘트만 시간순으로 반환된다.
  - [ ] 단위테스트: "최초 코멘트는 팀장부터" 규칙 위반(영업사원이 아무 코멘트 없는 일지에 먼저 답변 시도) 시 실패하는 테스트가 통과한다.
...
- **완료 조건**:
  - [ ] 영업일지 저장 시 팀장 이메일로(또는 콘솔 로그로) 알림이 발송/기록된다.
  - [ ] 코멘트/답변 등록 시 상대방에게 알림이 발송/기록된다.
  - [ ] 이메일 발송 함수가 강제로 예외를 던지도록 만들어도 영업일지/코멘트 저장 자체는 성공함을 테스트로 확인한다(RULE-NOTIFICATION-001).
  - [ ] 미가입 팀장 이메일로도(계정이 없어도) 발송 시도가 이루어진다(RULE-ORG-004).
...
- **완료 조건**:
  - [ ] 각 쿼리파라미터가 단독/조합으로 정상 필터링된다.
  - [ ] 단위테스트: 다른 사용자가 작성한 영업일지는 검색 결과에 절대 포함되지 않는다(RULE-SEARCH-001).
...
- **완료 조건**:
  - [ ] 응답에 코멘트/답변 데이터가 전혀 포함되지 않는다.
  - [ ] 삭제된 영업일지는 결과에서 제외된다.
  - [ ] 단위테스트: 조회자와 다른 팀장에게 매핑된 영업사원이 작성한 활동은 결과에서 제외됨을 확인한다(RULE-KNOWHOW-006).
...
- **완료 조건**:
  - [ ] 영업사원 2명 이상이 동일 이메일을 팀장 이메일로 입력해둔 상태에서 그 이메일로 팀장이 가입하면, 두 영업사원의 `manager_id`가 모두 새 팀장 ID로 업데이트된다.
  - [ ] 매칭 과정 중 하나라도 실패하면 전체가 롤백됨을 테스트로 확인한다(트랜잭션 원자성).
  - [ ] 매칭 후 팀장이 `GET /api/managed/sales-logs` 호출 시 기존에 쌓여있던 영업일지가 바로 조회된다.
...
- **완료 조건**:
  - [ ] 실제 BE-3 API와 연동해 회원가입/로그인이 동작한다.
  - [ ] 서버가 반환하는 검증 오류(사번 중복 등)가 화면에 표시된다.
  - [ ] 영업사원으로 로그인 시 영업일지 작성 화면으로, 팀장으로 로그인 시 팀장 View로 각각 이동한다.
